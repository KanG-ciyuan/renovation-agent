...
Proven by sending raw HTTP requests to the unmodified app:
...
- **Root cause:** `app.js:239-240` builds the static path with `path.join(publicDir, target)` and hands it to `fs.readFile` (`app.js:207`). There is no `path.resolve` + `startsWith(publicDir)` confinement check. `path.join` normalizes `..`; it does not reject it.
...
- **Remediation status.** The defect is **not fixed on `main`**. A verified fix exists on the `security/fix-public-path-traversal` branch: it resolves every request path against `public/`, rejects `..` segments, null bytes, encoded traversal and dot-prefixed names, and confirms the resolved path is still inside the public root before any filesystem read. It ships with a regression suite that fails 7 of 9 on the previous implementation and passes 9 of 9 on the fix. The fix does not reach `main` until that branch is merged, so until then this notice still applies.
...
| --- | --- |
| Reading a compact, dependency-free example of keyword/regex lead extraction with a deterministic scoring function | Any network-reachable deployment — the file-read defect is unfixed |
...
- **Filesystem reads are not confined** (see the [Security Notice](#security-notice)).
...
> **Do not tunnel this port, and do not put real credentials in `.env` while the file-read defect is open.** Keep `HOST` at `127.0.0.1`. See the [Security Notice](#security-notice).
