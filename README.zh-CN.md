...
- **根因：** `app.js:239-240` 用 `path.join(publicDir, target)` 拼接静态文件路径，并直接交给 `app.js:207` 的 `fs.readFile`。代码中没有 `path.resolve` + `startsWith(publicDir)` 的越界校验。`path.join` 会规范化 `..`，但不会拒绝它。
...
- **修复状态。** 该缺陷在 `main` 上**尚未修复**。`security/fix-public-path-traversal` 分支上已有一份经过验证的修复：把每个请求路径解析到 `public/` 之下，拒绝 `..` 片段、空字节、编码型穿越与点前缀文件名，并在任何文件系统读取之前确认解析结果仍位于 public 根目录内。该分支附带一套回归测试：在修复前的实现上 9 项中失败 7 项，在修复后 9 项全部通过。在该分支合并进 `main` 之前修复不会生效，因此在此之前本声明依然适用。
...
| 适合用来 | 不要用来 |
| --- | --- |
| 阅读一个紧凑、零依赖的关键词/正则线索提取示例，以及一个确定性评分函数 | 任何网络可达的部署 —— 文件读取缺陷尚未修复 |
...
> **不要给这个端口做隧道，也不要在文件读取缺陷未修复前往 `.env` 里放真实凭据。** 请保持 `HOST` 为 `127.0.0.1`。见[安全声明](#安全声明)。
